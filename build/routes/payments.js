...
var _default = router => {
  //YENİ BİR KARTLA KARTI KAYDETMEDEN ÖDEME YAP 
  router.post("/payments/:cartId/with-new-card", _Session.default, async (req, res) => {
    ...
    if (!req.params?.cartId) {
      throw new _ApiError.default("Cart is required", 400, "cartIdRequired");
    }
    ...
    const data = {
      ...
      coversationId: (0, _nanoid.default)(),
      price: paidPrice,
      paidPrice: paidPrice,
      currency: _iyzipay.default.CURRENCY.TRY,
      installments: '1',
      basketId: String(cart?._id),
      paymentChannel: _iyzipay.default.PAYMENT_CHANNEL.MOBILE_WEB,
  ...
  //YENİ BİR KARTLA KARTI KAYDEDEREK ÖDEME YAP
  ...
  //KAYITLI BİR KART İLE ÖDEME YAP / CARDİNDEX
  router.post("/payments/:cartId/:cardIndex/with-registered-card-index", _Session.default, async (req, res) => {
    ...
    if (index >= cards?.cardDetails?.length) {
      throw new _ApiError.default("Card dosent exists", 400, "cardIndexInvalid");
    }
    const {
      cardToken
  ...
  //KAYITLI BİR KART İLE ÖDEME YAP / CARDTOKEN
